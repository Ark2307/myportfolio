...
Topic: why one stack ends up running seven databases — each one is a different storage-engine bet, and the cost lands at write time, read time, or in disk layout.  
Layout: not a pipeline — a two-tier mapping. Row 1 (7 boxes): the databases you pick, each labelled with its use case (`APP DATA`, `RELATIONAL`, `SEARCH`, `CACHE`, `METRICS`, `ANALYTICS`, `WRITES`) and what it gives up (`trades joins`, `trades scale-out`, …). Row 2 (6 boxes): the storage engine underneath each one — B-Tree, Inverted Idx, Hash Slots, Time Series, Column Store, LSM-Tree — each showing its mechanism and where it pays. Every database drops into a shared bend row at `y=160` and turns into its engine; MongoDB and PostgreSQL converge on the same B-Tree box. Bottom row: three-costs badge / "what is it refusing to be good at" banner / failure-modes badge.  
Colors: box color is inherited from the engine it maps to — blue (B-Tree · MongoDB, PostgreSQL), green (Inverted Idx · Elasticsearch and Column Store · ClickHouse), red (Hash Slots · Redis), amber (Time Series · Prometheus), purple (LSM-Tree · Cassandra).  
Packets: one per database, colored by its engine, flowing top row → bend row → engine. The B-Tree drop trace carries two (MongoDB and PostgreSQL) to make the convergence visible.  
Note: the grid is inset to `x=26…774` rather than the usual `18…782`, because `BlogCard` applies `group-hover:scale-105` and would otherwise clip the outermost boxes' borders on hover.
